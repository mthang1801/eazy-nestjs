...
import {
  userSelector,
  getProductListByTradeinProgram,
...
import {
  ConverValuationBillDataFromAppcore,
  syncConvertValuationBillFromCms,
} from '../../utils/integrateFunctions';
...
export class TradeinProgramService {
  ...
  async createValuationBill(data) {
    ...
    try {
      ...
      if (!createCustomer) {
        ...
        if (checkUserAppcore) {
          createCustomer = await this.customerService.checkCustomerExistAndSave(
            data.customer_phone,
          );
        } else {
          let customerInfo = {
            ...
            b_phone: data.customer_phone,
            b_lastname: data.customer_name,
            phone: data.customer_phone,
          };
          createCustomer =
            await this.customerService.createCustomerFromWebPayment(
              customerInfo,
            );
        }
      }
  ...
  async logRequestSyncValuationBillToAppcore() {
    const findNullAppcore = await this.valuationBillRepo.find({
      appcore_id: IsNull(),
    });
    ...
    for (let item of findNullAppcore) {
      try {
        ...
        let criteriaSetList = await this.valuationBillCriteriaDetailRepo.find({
          valuation_bill_id: item.valuation_bill_id,
        });
        let criteria_set = [];
        for (let criteriaSet of criteriaSetList) {
          let set = {
            criteria_appcore_id: criteriaSet.criteria_appcore_id,
            criteria_detail_appcore_id: criteriaSet.criteria_detail_appcore_id,
          };
          criteria_set.push(set);
        }
        let price = await this.productPriceRepo.findOne({
  ...
  async turnOffTradeinProgram(tradein_id) {
    ...
    await this.logger.warn(
      `Timeout ${convertToSlug(tradeinProgram.name)} deleted!`,
    );
  }
  ...
  async addTimeoutTurnOnTradeinProgram(tradein_id) {
    ...
    const timeout = setTimeout(callback, milliseconds);
    console.log(
      'tradein program will start after ' + milliseconds / 3600000 + ' hours.',
    );
    this.schedulerRegistry.addTimeout(
      convertToSlug(tradeinProgram.name),
      timeout,
